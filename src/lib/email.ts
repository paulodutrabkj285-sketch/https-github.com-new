...
function separarNomeContato(nome: string) {
  const nomeLimpo = String(nome || "Cliente")
    .trim()
    .replace(/\s+/g, " ");

  const partes = nomeLimpo.split(" ");
  const firstName = partes.shift() || "Cliente";
  const lastName = partes.join(" ");

  return { firstName, lastName };
}

async function garantirContatoRespondIo({
  token,
  telefone,
  nome,
}: {
  ...
  const identificador = `phone:${telefone}`;
  const { firstName, lastName } = separarNomeContato(nome);

  const endpoint =
    `https://api.respond.io/v2/contact/create_or_update/${encodeURIComponent(
      identificador
    )}`;

  console.log("RESPOND.IO - CRIANDO/ATUALIZANDO CONTATO", {
    identificador,
    firstName,
    lastName,
  });
  ...
  if (!resposta.ok) {
    throw new Error(
      `Falha ao criar/atualizar contato no respond.io. HTTP ${resposta.status}: ${texto || "sem resposta"
      }`
    );
  }

  console.log("RESPOND.IO - CONTATO DISPONÍVEL", {
    telefone,
...
function montarPayloadIngressoWhatsApp({
  payloadBaseRaw,
  nome,
  dataVisita,
  codigoIngresso,
  ...
  if (!payload?.message || payload.message.type !== "whatsapp_template") {
    throw new Error(
      "Payload do respond.io inválido: message.type deve ser whatsapp_template."
    );
  }
  ...
  if (template.name !== "ingresso_parque_mundo_novo") {
    console.warn("Template inesperado:", template.name);
  }
  ...
  if (!header) {
    throw new Error("Template não possui HEADER.");
  }

  if (!Array.isArray(header.parameters) || header.parameters.length === 0) {
    throw new Error("HEADER não possui parâmetro de documento.");
  }
  ...
  header.parameters[0] = {
    ...
    document: {
      link: pdfUrl,
      caption: `Ingresso ${codigoIngresso} - Parque Mundo Novo`,
    },
  };
  ...
  if (!body) {
    throw new Error("Template não possui BODY.");
  }

  const dataFormatada = formatarData(dataVisita) || "Não informada";

  body.parameters = [
    { type: "text", text: String(nome || "Cliente").trim() },
    { type: "text", text: dataFormatada },
    { type: "text", text: codigoIngresso },
  ];

  if (typeof body.text === "string") {
    body.text = body.text
      .replaceAll("{{1}}", String(nome || "Cliente").trim())
...
async function enviarIngressoRespondIo({
  telefone,
  nome,
  dataVisita,
  codigoIngresso,
  ...
  if (!token) {
    console.warn("RESPOND_IO_TOKEN não configurado.");
    return { enviado: false, motivo: "token_ausente" };
  }

  const payloadBaseRaw = process.env.RESPOND_IO_TEMPLATE_PAYLOAD?.trim();

  if (!payloadBaseRaw) {
    console.warn("RESPOND_IO_TEMPLATE_PAYLOAD não configurado.");
    return { enviado: false, motivo: "template_payload_ausente" };
  }

  if (!pdfUrl || !/^https:\/\/.+/i.test(pdfUrl)) {
    return { enviado: false, motivo: "pdf_url_ausente" };
  }

  const telefoneNormalizado = normalizarTelefoneWhatsApp(telefone);
  const identificador = `phone:${telefoneNormalizado}`;

  await garantirContatoRespondIo({
    token,
    telefone: telefoneNormalizado,
    nome,
  });

  const payload = montarPayloadIngressoWhatsApp({
    payloadBaseRaw,
    nome,
    dataVisita,
    codigoIngresso,
  ...
  const endpoint =
    `https://api.respond.io/v2/contact/${encodeURIComponent(
      identificador
    )}/message`;

  console.log("RESPOND.IO - ENVIANDO INGRESSO", {
    pedidoId: pedidoId || null,
    telefone: telefoneNormalizado,
    codigoIngresso,
    template: payload?.message?.template?.name,
  ...
  if (!resposta.ok) {
    console.error("RESPOND.IO - ERRO AO ENVIAR INGRESSO", {
      pedidoId: pedidoId || null,
      telefone: telefoneNormalizado,
      codigoIngresso,
      status: resposta.status,
    ...
    throw new Error(
      `Erro respond.io (${resposta.status}): ${typeof respostaJson === "string"
        ? respostaJson
        : JSON.stringify(respostaJson)
      }`
    );
  }

  console.log("RESPOND.IO - INGRESSO ENVIADO", {
    pedidoId: pedidoId || null,
    telefone: telefoneNormalizado,
    codigoIngresso,
    status: resposta.status,
...
async function enviarIngressoPorWhatsApp({
  ...
  return enviarIngressoRespondIo({
    telefone,
    nome,
    dataVisita,
    codigoIngresso,
...
/* =========================================================
   WHATSAPP - REENVIO MANUAL PELO PAINEL
========================================================= */

type EnviarIngressoWhatsappManualParams = {
  telefone: string;
  nome: string;
  dataVisita: string;
  codigoIngresso: string;
...
export async function enviarIngressoPorWhatsapp({
  ...
  pdfUrl,
  pedidoId,
}: EnviarIngressoWhatsappManualParams) {
  return enviarIngressoRespondIo({
    telefone,
    nome,
    dataVisita,
