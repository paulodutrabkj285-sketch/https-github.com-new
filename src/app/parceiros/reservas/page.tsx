...
const WHATSAPP_PARQUE = "5549991299991";
...
export default function ReservaParceiroPage() {
  ...
  function gerarLinkWhatsApp(codigoGrupo: string) {
    const texto = `
🏞️ NOVA RESERVA DE AGÊNCIA

Código do grupo: ${codigoGrupo}

📅 Data da visita: ${formatarData(dataVisita)}
🕘 Chegada prevista: ${horaPrevista || "Não informada"}
🚌 Tipo de veículo: ${tipoVeiculo}

👨 Adultos: ${adultos}
👵 Idosos / meia entrada: ${idosos}
🚡 Elevador: ${temElevador ? `Sim (${qtdElevador})` : "Não"}

👥 Total de visitantes: ${calculo.totalVisitantes}

💰 Valor bruto: ${formatarMoeda(calculo.valorBruto)}
🏷️ Desconto aplicado: ${formatarMoeda(calculo.valorDesconto)}
✅ Valor final: ${formatarMoeda(calculo.valorFinal)}

💳 Pagamento: na chegada ao parque

📝 Observações:
${observacoes || "Nenhuma observação informada."}
`;

    return `https://wa.me/${WHATSAPP_PARQUE}?text=${encodeURIComponent(texto)}`;
  }
  ...
  async function salvarReserva() {
    ...
    try {
      ...
      const whatsappReserva = gerarLinkWhatsApp(codigoGrupo);
      ...
      await addDoc(collection(db, "reservas_agencias"), {
        ...
        whatsappParque: WHATSAPP_PARQUE,
        whatsappReserva,
        enviadoWhatsAppParque: true,

        createdAt: serverTimestamp(),
        updatedAt: serverTimestamp(),
      });

      setLinkWhatsApp(whatsappReserva);

      setMensagem(
        `Reserva criada com sucesso! Código do grupo: ${codigoGrupo}. O pagamento será realizado na chegada ao parque.`
      );

      window.open(whatsappReserva, "_blank");
        ...
        <div className="max-w-5xl mx-auto">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <aside className="bg-white/95 text-slate-900 rounded-2xl p-6 shadow-xl h-fit backdrop-blur">
              ...
              <div className="mt-4 rounded-xl bg-yellow-50 border border-yellow-100 p-4 text-xs text-yellow-800">
                <p className="font-bold">Pagamento na chegada</p>
                <p className="mt-1">
                  A excursão será reservada agora e o pagamento será recebido
                  pelo parque no dia da visita.
                </p>
              </div>
            </aside>
          </div>
